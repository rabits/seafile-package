...
class Project(object):
    ...
    def __init__(self):
        ...
        self.src = os.path.join(conf[CONF_SRCDIR], self.name)
        # project dir, like <builddir>/seafile-1.2.2/
        self.projdir = os.path.join(conf[CONF_BUILDDIR], self.name)

    def copy(self):
        '''Copy the source from the dir'''
        info('Copying %s' % self.name)

        if run('cp -a "%s" .' % self.src) != 0:
            error('failed to copy source of %s' % self.name)
...
class Ccnet(Project):
    ...
    def __init__(self):
        ...
        self.build_commands = [
            './autogen.sh',
            './configure --prefix=%s --disable-client --enable-server --disable-compile-demo --enable-pgsql --enable-ldap' % self.prefix,
            'make',
            'make install'
        ]
...
class Seafile(Project):
    ...
    def __init__(self):
        ...
        self.build_commands = [
            './autogen.sh',
            './configure --prefix=%s --disable-client --enable-server --enable-pgsql' % self.prefix,
            'make',
            'make install'
        ]
...
def parse_args():
    ...
    parser.add_option(long_opt(CONF_THIRDPARTDIR), dest=CONF_THIRDPARTDIR, nargs=1, help='where to find the thirdpart libs for seahub')
    parser.add_option(long_opt(CONF_VERSION), dest=CONF_VERSION, nargs=1, help='the version to build. Must be digits delimited by dots, like 1.3.0')
    parser.add_option(long_opt(CONF_BUILDDIR), dest=CONF_BUILDDIR, nargs=1, help='the directory to build the source. Defaults to /tmp', default=tempfile.gettempdir())
    parser.add_option(long_opt(CONF_OUTPUTDIR), dest=CONF_OUTPUTDIR, nargs=1, help='the output directory to put the generated server tarball. Defaults to the current directory.', default=os.getcwd())
    parser.add_option(long_opt(CONF_SRCDIR), dest=CONF_SRCDIR, nargs=1, help='''Source must be placed in this directory.''')
    parser.add_option(long_opt(CONF_KEEP), dest=CONF_KEEP, action='store_true', help='''keep the build directory after the script exits. By default, the script would delete the build directory at exit.''')
    parser.add_option(long_opt(CONF_NO_STRIP), dest=CONF_NO_STRIP, action='store_true', help='''do not strip debug symbols''')
...
def setup_build_env():
    ...
    prepend_env_value('CPPFLAGS', '-I%s' % os.path.join(prefix, 'include'), seperator=' ')
    prepend_env_value('CPPFLAGS', '-DLIBICONV_PLUG', seperator=' ')

    if conf[CONF_NO_STRIP]:
        prepend_env_value('CPPFLAGS', '-g -O0', seperator=' ')
        prepend_env_value('CFLAGS', '-g -O0', seperator=' ')

    prepend_env_value('LDFLAGS', '-L%s' % os.path.join(prefix, 'lib'), seperator=' ')
    prepend_env_value('LDFLAGS', '-L%s' % os.path.join(prefix, 'lib64'), seperator=' ')
...
def copy_seafdav():
    ...
    source = os.path.join(conf[CONF_SRCDIR], 'seafdav', 'wsgidav')
    if run('cp -a "%s" "%s"' % (source, dst_dir)) != 0:
        error('failed to copy %s' % source)

    dst_dir = os.path.join(conf[CONF_BUILDDIR], 'seafile-server', 'seahub', 'thirdpart')
    source = os.path.join(conf[CONF_SRCDIR], 'seafobj', 'seafobj')
    if run('cp -a "%s" "%s"' % (source, dst_dir)) != 0:
        error('failed to copy %s' % source)
...
def copy_scripts_and_libs():
    ...
    must_copy(os.path.join(scripts_srcdir, 'setup-seafile-mysql.sh'), serverdir)
    must_copy(os.path.join(scripts_srcdir, 'setup-seafile-mysql.py'), serverdir)
...
def copy_shared_libs():
    ...
    seafile_path = os.path.join(builddir, 'seafile-server', 'seafile', 'bin', 'seaf-server')
    ccnet_server_path = os.path.join(builddir, 'seafile-server', 'seafile', 'bin', 'ccnet-server')
    seaf_fuse_path = os.path.join(builddir, 'seafile-server', 'seafile', 'bin', 'seaf-fuse')
